...
static int bq2589x_psy_register(struct bq2589x *bq)
{
	...
	bq->usb_desc.name = "bq2589x-usb";
	bq->usb_desc.type = POWER_SUPPLY_TYPE_USB;
	bq->usb_desc.properties = bq2589x_charger_props;
	bq->usb_desc.num_properties = ARRAY_SIZE(bq2589x_charger_props);
	bq->usb_desc.get_property = bq2589x_usb_get_property;
	bq->usb_desc.external_power_changed = NULL;

	bq->usb = power_supply_register(bq->dev, &bq->usb_desc, &psy_cfg);
	if (bq->usb < 0) {
		dev_err(bq->dev, "%s:failed to register usb psy:%d\n", __func__, ret);
		return ret;
	}

	bq->wall_desc.name = "bq2589x-Wall";
	bq->wall_desc.type = POWER_SUPPLY_TYPE_MAINS;
	bq->wall_desc.properties = bq2589x_charger_props;
	bq->wall_desc.num_properties = ARRAY_SIZE(bq2589x_charger_props);
	bq->wall_desc.get_property = bq2589x_wall_get_property;
	bq->wall_desc.external_power_changed = NULL;

	bq->wall = power_supply_register(bq->dev, &bq->wall_desc, &psy_cfg);
	if (bq->wall < 0) {
			   ...
			   const struct i2c_device_id *id)
{
	...
	if (!ret && bq->part_no == BQ25895) {
		bq->status |= BQ2589X_STATUS_EXIST;
		dev_info(bq->dev, "%s: charger device bq2589x detected, revision:%d\n", __func__, bq->revision);
	} else {
